...
/** Resume hub shipped; older DB rows may still have `soon` until migrate/seed runs. */
function catalogStatusForApi(id, status) {
  if (id === "resume-ai" && status === "soon") return "live";
  return status;
}
...
export async function getFeatures() {
  ...
  try {
    ...
    return rows.map((r) => ({
      ...
      status: catalogStatusForApi(r.id, r.status),
    }));
  } catch (err) {
...
export async function patchFeature(id, patch) {
  ...
  return {
    ...
    status: catalogStatusForApi(updated.id, updated.status),
  };
}
